...
function MainPageKCS({ }: PropsMainPageKCS) {
	...
	const listDebtBill = useQuery(
		[QUERY_KEY.table_cong_no_phieu_da_kcs, _page, _pageSize, _keyword, _partnerUuid, _userUuid, _dateFrom, _dateTo],
		{
			queryFn: () =>
				httpRequest({
					isList: true,
					...
					<Table
						...
						column={[
							...
							},
							{
								...
								render: (data: IDebtBill) => (
									<>{data?.status == STATUS_BILL.DA_KCS ? convertCoin(data?.moneyTotal) : '---'}</>
								),
							},
							{
								title: 'Ngày nhập',
