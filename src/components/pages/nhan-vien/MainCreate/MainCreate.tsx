...
import {IFormCreate, PropsMainCreate} from './interfaces';
...
function MainCreate({}: PropsMainCreate) {
	...
	const funCreateUser = useMutation({
		mutationFn: () =>
			httpRequest({
	...
	return (
		<div className={styles.container}>
			<Loading loading={funCreateUser.isLoading} />
			<div className={styles.form}>
				<Form form={form} setForm={setForm} onSubmit={handleSubmit}>
					...
					<div className={clsx('mt')}>
						<TextArea max={5000} placeholder='Nhập ghi chú' name='description' label={<span>Ghi chú</span>} blur />
					</div>
				</Form>
			</div>
		</div>
